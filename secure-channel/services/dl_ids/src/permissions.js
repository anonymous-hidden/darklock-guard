...
export function resolvePermissions({ userId, serverId, channelId, db }) {
  ...
  if (!server) {
    securityDebugWarn('[permissions] resolvePermissions: server NOT FOUND serverId=%s userId=%s', serverId, userId);
    return { permissions: 0n, isOwner: false, isAdmin: false, notFound: true };
  }

  if (server.owner_id === userId) {
    securityDebugLog('[permissions] resolvePermissions: OWNER userId=%s serverId=%s => ALL_PERMISSIONS', userId, serverId);
  ...
  securityDebugLog('[permissions] resolvePermissions: userId=%s serverId=%s channelId=%s roles=%d', userId, serverId, channelId, userRoles.length);

  if (userRoles.length === 0) {
    securityDebugWarn('[permissions] resolvePermissions: NO ROLES (not a member?) userId=%s serverId=%s', userId, serverId);
  ...
  if (isAdmin) {
    securityDebugLog('[permissions] resolvePermissions: ADMIN userId=%s serverId=%s => ALL_PERMISSIONS', userId, serverId);
  ...
  if (!channelId) {
    securityDebugLog('[permissions] resolvePermissions: SERVER-LEVEL userId=%s serverId=%s perms=%s', userId, serverId, basePerms.toString());
