...
export function requireChannelPermission(permissionKey) {
  return (req, res, next) => {
    try {
      ...
      console.error('[channel-rbac-mw] requireChannelPermission error:');
...
export function requireSecureChannelAccess(permissionKey, action, opts = {}) {
  return (req, res, next) => {
    try {
      ...
      console.error('[channel-rbac-mw] requireSecureChannelAccess error:');
      res.status(500).json({ error: 'Internal permission check error', code: 'internal' });
    }
  };
}
...
export function requireSecurityRole(minLevel) {
  return (req, res, next) => {
    try {
      ...
      console.error('[channel-rbac-mw] requireSecurityRole error:');
      res.status(500).json({ error: 'Internal permission check error', code: 'internal' });
    }
  };
}
...
export function requireSecureChannelOnly() {
  return (req, res, next) => {
    try {
      ...
      console.error('[channel-rbac-mw] requireSecureChannelOnly error:');
