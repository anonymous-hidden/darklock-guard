...
/** Broadcast account-level events only to recipients approved by the caller. */
export function broadcastGlobalWhere(event, data, shouldDeliver, excludeUserId = null) {
  if (!globalClients.size) return;
  const payload = `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`;
  for (const client of globalClients) {
    if (excludeUserId && client.userId === excludeUserId) continue;
    try {
      if (!shouldDeliver(client.userId)) continue;
      client.res.write(payload);
    } catch {}
  }
}
