...
function hashInviteToken(token) {
  return `sha256:${crypto.createHash('sha256').update(String(token || '').trim()).digest('hex')}`;
}
...
function createRateLimiter({ windowMs, max, message }) {
  ...
  return (req, res, next) => {
    const key = String(req.ip ?? req.socket?.remoteAddress ?? 'unknown').trim().toLowerCase() || 'unknown';
    const now = Date.now();
...
const invitePreviewLimiter = createRateLimiter({
  windowMs: 60 * 1000,
  max: 60,
...
invitesRouter.post('/:id/invites', requireAuth, (req, res) => {
  try {
    ...
    let expiresAt = new Date(Date.now() + 7 * 86400000).toISOString();

    if (expires_in) {
      // Accept named durations OR numeric seconds (e.g. "3600" or 3600)
      const secs = Number(expires_in);
      if (!isNaN(secs) && secs > 0) {
        expiresAt = new Date(Date.now() + Math.min(secs, 30 * 86400) * 1000).toISOString();
      ...
      // Invalid or non-expiring values retain the secure seven-day default.
    }
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(inviteId, serverId, userId, hashInviteToken(token), expiresAt, max_uses || 0);

    auditLog(db, serverId, userId, 'INVITE_CREATE', 'invite', inviteId, { max_uses, expires_in });
    ...
    console.error('[IDS_INVITE_CREATE_FAILED]');
...
invitesRouter.get('/:id/invites', requireAuth, (req, res) => {
  try {
    ...
    res.json({
      invites: invites.map(({ token: _tokenDigest, ...invite }) => invite),
    });
  } catch (err) {
    console.error('[IDS_INVITE_LIST_FAILED]');
...
invitesRouter.delete('/:id/invites/:invId', requireAuth, (req, res) => {
  try {
    ...
    console.error('[IDS_INVITE_DELETE_FAILED]');
...
invitesRouter.get('/:token', (req, res) => {
  try {
    ...
    console.error('[IDS_INVITE_READ_FAILED]');
...
invitesRouter.get('/:token/preview', invitePreviewLimiter, (req, res) => {
  try {
    ...
    console.error('[IDS_INVITE_PREVIEW_FAILED]');
...
invitesRouter.post('/:token/join', requireAuth, (req, res) => {
  try {
    ...
    const invite = db.prepare('SELECT * FROM server_invites WHERE token = ?').get(hashInviteToken(token));
    if (!invite) return res.status(404).json({ error: 'Invite not found', code: 'not_found' });
    ...
    auditLog(db, invite.server_id, userId, 'MEMBER_JOIN', 'user', userId, { via_invite: true });
    ...
    console.error('[IDS_INVITE_JOIN_FAILED]');
