...
const DEFAULT_TAG_ADMIN_IDS = ['dev-user-0000', 'ridgeline-user-one'];

function getGrantedTags(db, userId) {
  return db.prepare(`
...
function getSelectedTags(db, userId) {
  return db.prepare(`
...
function isTagAdmin(req) {
  const configuredIds = (process.env.IDS_TAG_ADMIN_USER_IDS ?? DEFAULT_TAG_ADMIN_IDS.join(','))
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);
  if (configuredIds.includes(req.userId)) return true;

  const actor = req.db.prepare('SELECT system_role FROM users WHERE id = ?').get(req.userId);
  const systemRole = String(actor?.system_role ?? '').trim().toLowerCase();
  return ['owner', 'co_owner', 'admin', 'developer'].includes(systemRole);
}

function requireTagAdmin(req, res, next) {
  if (!isTagAdmin(req)) {
...
tagsRouter.get('/users/me/tags', requireAuth, (req, res) => {
  try {
    ...
    const granted = getGrantedTags(db, userId);
    const selected = getSelectedTags(db, userId);
    res.json({ max_selected: MAX_SELECTED_TAGS, granted, selected });
  } catch (err) {
    console.error('[tags] get me tags error:');
...
tagsRouter.put('/users/me/tags/selected', requireAuth, (req, res) => {
  try {
    ...
    console.error('[tags] update selection error:');
...
tagsRouter.get('/users/:id/tags', requireAuth, (req, res) => {
  try {
    ...
    console.error('[tags] get user tags error:');
    res.status(500).json({ error: 'Failed to load user tags', code: 'internal' });
  }
});

// ── GET /admin/users/:id/tags ──────────────────────────────────────────────
tagsRouter.get('/admin/users/:id/tags', requireAuth, requireTagAdmin, (req, res) => {
  try {
    const db = req.db;
    const userId = req.params.id;
    ...
    res.json({
      ...
      granted: getGrantedTags(db, userId),
      selected: getSelectedTags(db, userId),
    });
  } catch (err) {
    console.error('[tags] admin get tags error:');
    res.status(500).json({ error: 'Failed to load tags', code: 'internal' });
  }
});

// ── POST /admin/users/:id/tags/:tagKey ─────────────────────────────────────
tagsRouter.post('/admin/users/:id/tags/:tagKey', requireAuth, requireTagAdmin, (req, res) => {
  try {
    const db = req.db;
    const userId = req.params.id;
    ...
    const user = db.prepare('SELECT id FROM users WHERE id = ?').get(userId);
    if (!user) return res.status(404).json({ error: 'User not found', code: 'not_found' });
    const tag = db.prepare('SELECT id FROM app_tags WHERE key = ?').get(tagKey);
    if (!tag) return res.status(404).json({ error: 'Tag not found', code: 'not_found' });

    db.prepare(`
      INSERT OR IGNORE INTO user_tag_grants (id, user_id, tag_id, granted_by)
      VALUES (?, ?, ?, ?)
    `).run(randomUUID(), userId, tag.id, `admin:${req.userId}`);
    ...
    console.error('[tags] admin grant tag error:');
    res.status(500).json({ error: 'Failed to grant tag', code: 'internal' });
  }
});

// ── DELETE /admin/users/:id/tags/:tagKey ───────────────────────────────────
tagsRouter.delete('/admin/users/:id/tags/:tagKey', requireAuth, requireTagAdmin, (req, res) => {
  try {
    const db = req.db;
    const userId = req.params.id;
    const tagKey = req.params.tagKey;
    ...
    const tx = db.transaction(() => {
      db.prepare('DELETE FROM user_tag_selections WHERE user_id = ? AND tag_id = ?').run(userId, tag.id);
      db.prepare('DELETE FROM user_tag_grants WHERE user_id = ? AND tag_id = ?').run(userId, tag.id);
    });
    tx();
    ...
    console.error('[tags] admin revoke tag error:');
    res.status(500).json({ error: 'Failed to revoke tag', code: 'internal' });
  }
});
...
tagsRouter.post('/internal/tags/create', requireAppGrantAuth, (req, res) => {
  try {
    ...
    console.error('[tags] create tag error:');
...
tagsRouter.post('/internal/tags/grant', requireAppGrantAuth, (req, res) => {
  try {
    ...
    console.error('[tags] grant tag error:');
    res.status(500).json({ error: 'Failed to grant tag', code: 'internal' });
  }
});
