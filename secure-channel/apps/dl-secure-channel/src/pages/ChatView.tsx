...
export default function ChatView() {
  ...
  useEffect(() => {
    if (!sessionId) return;
    getMessages(sessionId, 50).then((msgs) => setMessages(sessionId, msgs)).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
  }, [sessionId]);
  ...
  const handleTrust = async () => {
    ...
    try {
      ...
      console.error('[DM_VERIFY_FAILED]');
    } finally {
      setVerifying(false);
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-dl-muted max-w-xs">
            Choose a contact from the sidebar to start a direct message.
          </p>
          {RIDGELINE_SECURITY_CAPABILITIES.dmE2eeSupported && (
            <div className="flex items-center justify-center gap-1.5 text-xs text-dl-accent">
              <Lock size={12} />
              <span>Direct E2E</span>
            </div>
          )}
        </div>
      </div>
