...
export const useServerStore = create<ServerState>((set, get) => ({
  ...
  // ── Servers ─────────────────────────────────────────────────────
  fetchServers: async () => {
    console.log('[SERVER_LIST_FETCH_STARTED]');
    set({ loading: true });
    try {
      const servers = await api.getServers();
      console.log('[SERVER_LIST_FETCH_COMPLETED]');
      set({ servers });
      ...
      console.log('[SERVER_ACTIVE_SELECTION_CHECKED]');
      if (activeServerId && !servers.find((s) => s.id === activeServerId)) {
        ...
        if (isLocalGroup) {
          console.log('[SERVER_LOCAL_GROUP_SELECTION_PRESERVED]');
        } else {
          console.warn('[SERVER_STALE_SELECTION_CLEARED]');
          setActiveServer(null);
        }
      }
    } catch (e) {
      console.error('[SERVER_LIST_FETCH_FAILED]');
    } finally {
      set({ loading: false });
    }
  },
  ...
  createServer: async (name, description) => {
    console.log('[SERVER_CREATE_STARTED]');
    const server = await api.createServer(name, description);
    console.log('[SERVER_CREATE_COMPLETED]');
  ...
  reorderChannels: async (serverId, channelsLayout) => {
    ...
    try {
      ...
      console.error('[CHANNEL_REORDER_FAILED]');
  ...
  fetchServerUnread: async (serverId) => {
    try {
      ...
      console.error('[SERVER_UNREAD_FETCH_FAILED]');
    }
  },
  ...
  fetchMentionNotifications: async (limit = 50) => {
    try {
      ...
      console.error('[SERVER_MENTIONS_FETCH_FAILED]');
    }
  },

  // ── Roles ───────────────────────────────────────────────────────
  fetchRoles: async (serverId) => {
    console.log('[ROLE_LIST_FETCH_STARTED]');
    try {
      const roles = await api.getRoles(serverId);
      // Sort by position descending (highest first)
      roles.sort((a, b) => b.position - a.position);
      console.log('[ROLE_LIST_FETCH_COMPLETED]');
      set((s) => ({ roles: { ...s.roles, [serverId]: roles } }));
    } catch (e) {
      console.error('[ROLE_LIST_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchMembers: async (serverId) => {
    console.log('[MEMBER_LIST_FETCH_STARTED]');
    try {
      const members = await api.getServerMembers(serverId);
      console.log('[MEMBER_LIST_FETCH_COMPLETED]');
      set((s) => ({ members: { ...s.members, [serverId]: members } }));
    } catch (e) {
      console.error('[MEMBER_LIST_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchOverrides: async (serverId, channelId) => {
    console.log('[CHANNEL_OVERRIDE_FETCH_STARTED]');
    try {
      const ov = await api.getChannelOverrides(serverId, channelId);
      console.log('[CHANNEL_OVERRIDE_FETCH_COMPLETED]');
      set((s) => ({ overrides: { ...s.overrides, [channelId]: ov } }));
    } catch (e) {
      console.error('[CHANNEL_OVERRIDE_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchAuditLog: async (serverId, opts) => {
    console.log('[AUDIT_LOG_FETCH_STARTED]');
    try {
      ...
      console.log('[AUDIT_LOG_FETCH_COMPLETED]');
      set((s) => ({
        auditLog: {
          ...s.auditLog,
      ...
      console.error('[AUDIT_LOG_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchInvites: async (serverId) => {
    try {
      ...
      console.error('[INVITE_LIST_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchAutoModRules: async (serverId) => {
    try {
      ...
      console.error('[AUTOMOD_RULES_FETCH_FAILED]');
      throw e;
    }
  },
  ...
  fetchAutoModEvents: async (serverId, limit) => {
    try {
      ...
      console.error('[AUTOMOD_EVENTS_FETCH_FAILED]');
      throw e;
    }
  },
