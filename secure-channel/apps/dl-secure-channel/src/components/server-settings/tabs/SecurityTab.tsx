...
export default function SecurityTab({ serverId }: { serverId: string }) {
  ...
  useEffect(() => {
    fetchAutoModRules(serverId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
    fetchAutoModEvents(serverId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
  ...
  const handleToggle = async (rule: AutoModRuleDto) => {
    try {
      ...
      console.error("Toggle rule failed:");
    }
  };
...
const ENCRYPTION_MODES: { value: EncryptionMode; label: string; desc: string; icon: typeof Lock }[] = [
  { value: "standard", label: "Direct Messages", desc: "Current direct messages use E2EE; group messaging is paused", icon: Lock },
  { value: "enforced_e2e", label: "Unavailable", desc: "Group end-to-end encryption is not deployed", icon: ShieldCheck },
  { value: "hybrid", label: "Unavailable", desc: "Mixed channel encryption is not deployed", icon: ShieldAlert },
];
        ...
        <div className="grid grid-cols-3 gap-2">
          {ENCRYPTION_MODES.map(({ value, label, desc, icon: Icon }) => (
            <button
              key={value}
              onClick={() => {
                if (value === "standard" || RIDGELINE_SECURITY_CAPABILITIES.groupE2eeSupported) {
                  setEncryptionMode(value);
                }
              }}
              disabled={value !== "standard" && !RIDGELINE_SECURITY_CAPABILITIES.groupE2eeSupported}
