  ...
  const handleSend = async () => {
    ...
    if (text.startsWith("/")) {
      const ctx = {
        ...
        serverId: sessionId, // In server context, this is the server channel
        channelId: sessionId,
        userPermissions: 0xFFFFFFFF, // TODO: wire real permissions
        userRoles: [] as string[],
        isOwner: false,
        isAdmin: true, // TODO: check real admin status
      };
    ...
    try {
      console.log('[DM_SEND_STARTED]');
      const msg = await sendMessage(sessionId, text);
      replaceMessage(sessionId, tempId, { ...msg, delivery_state: "delivered" });
      console.log('[DM_SEND_ACCEPTED]');
    } catch (err) {
      const rawMsg = err instanceof Error ? err.message : String(err);
      const lower = rawMsg.toLowerCase();
  ...
  return (
    <div className="message-input-wrapper">
      ...
      <div className={`message-input ${disabled ? "message-input--disabled" : ""}`}>
        ...
        >
          {attachSending ? <Loader2 size={20} className="animate-spin" /> : <Plus size={20} />}
        </button>

        {/* Markdown toggle */}
        <button
          className={`message-input__md-toggle ${showMarkdownBar ? "message-input__md-toggle--active" : ""}`}
          title="Markdown toolbar"
          onClick={() => setShowMarkdownBar(!showMarkdownBar)}
          disabled={disabled}
        >
          <Hash size={16} />
        </button>
        ...
        <div className="message-input__right">
          <div className="relative" ref={emojiRef}>
            ...
            {/* ── Emoji Picker ─────────────────────────────────────── */}
            {showEmoji && (
              <div className="absolute bottom-10 right-0 w-[320px] bg-[#111218] border border-white/[0.08] rounded-xl shadow-2xl shadow-black/60 z-50 overflow-hidden">
                {/* Category tabs */}
