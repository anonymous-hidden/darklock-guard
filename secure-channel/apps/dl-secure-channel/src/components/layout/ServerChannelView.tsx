...
export default function ServerChannelView() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      console.error('[SERVER_CHANNEL_FETCH_FAILED]');
    }
  ...
  useEffect(() => {
    ...
    if (serverId) {
      fetchMembers(serverId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
      fetchRoles(serverId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
    }

    // Connect WS gateway if not connected, then subscribe to this channel
    if (!wsConnected) {
      wsConnect().catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
    }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      console.error('[GROUP_SEND_BLOCKED_OR_FAILED]');
      setBody(text);
    } finally {
      setSending(false);
      textareaRef.current?.focus();
  ...
  const handleDelete = async (msgId: string) => {
    ...
    try {
      ...
      console.error('[GROUP_DELETE_BLOCKED_OR_FAILED]');
    }
  };
