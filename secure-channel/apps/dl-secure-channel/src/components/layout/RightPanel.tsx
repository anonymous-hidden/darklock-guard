...
export default function RightPanel() {
  ...
  useEffect(() => {
    if (rightPanelOpen && sidebarView === "server" && activeServerId) {
      fetchMembers(activeServerId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
      fetchRoles(activeServerId).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
    }
...
function ServerMemberList({
  ...
  useEffect(() => {
    ...
    if (userIds.length > 0) {
      fetchBatchPresence(userIds).catch(() => console.error('[RIDGELINE_ASYNC_OPERATION_FAILED]'));
    }
