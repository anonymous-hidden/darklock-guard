check "production_apply_guard" {
  assert {
    condition = !var.deployment_enabled || (
      can(regex("^CHG-[0-9]{6,}$", var.production_change_approval)) &&
      !var.migration_enabled
    )
    error_message = "Production requires an approved CHG identifier, and migration access must remain disabled for the foundation apply."
  }
}

resource "terraform_data" "production_apply_guard" {
  count = var.deployment_enabled ? 1 : 0

  input = var.production_change_approval
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Production apply blocked: approved CHG identifier required and migration access must be disabled."
    }
  }
}
...
module "security" {
  ...
  depends_on = [terraform_data.production_apply_guard]
}
