...
class AntiPhishing {
    constructor(bot) {
        ...
        this.suspiciousPatterns = [
            // Discord impersonation patterns (only suspicious with a URL present)
            ...
            /act[\s\-_]*fast/i,
            /expires[\s\-_]*soon/i,
            // NOTE: 'congratulations' removed as standalone pattern — too many false positives
            // when users send congratulations GIFs/reactions. Only flag when combined with a URL.
            /you[\s\-_]*won/i,
            ...
            // Technical phishing (only flag installs/downloads with a URL attached)
            /account[\s\-_]*suspended/i
        ];
        
        // Patterns that only trigger when the message ALSO contains a URL
        this.urlRequiredPatterns = [
            /congratulations/i,
            /download[\s\-_]*now/i,
    ...
    async checkMessageContent(message, config = {}) {
        ...
        // URL-required patterns: only flag if message also contains a link
        if (hasUrl) {
            for (const pattern of this.urlRequiredPatterns) {
                if (pattern.test(content)) {
                    issues.push({
                        type: 'PHISHING_CONTENT',
                        severity: 'HIGH',
                        details: `Message contains suspicious term with a URL`,
                        confidence: 0.75,
                        pattern: pattern.source
                    });
                }
