...
class AntiNuke {
    ...
    async handleViolation(guild, userId, violation) {
        ...
        // FORENSIC BACKUP: Capture the current (possibly damaged) server state
        // immediately so evidence of the attack is preserved before restoration.
        // Fire-and-forget — don't block the response pipeline.
        if (this.bot.serverBackup) {
            this.bot.serverBackup.createBackup(guild.id, {
                type: 'auto',
                description: `⚠️ Pre-restore snapshot — anti-nuke triggered (${violation.actionType})`,
                createdBy: 'antinuke-system',
                includeRoles:    true,
        ...
        try {
            ...
            // POST-RESTORE BACKUP: Create a clean snapshot of the fully-restored
            // server so the dashboard always reflects the current good state.
            if (this.bot.serverBackup) {
                this.bot.serverBackup.createBackup(guild.id, {
                    type: 'auto',
                    description: `✅ Post-restore snapshot — incident ${incident.id}`,
                    createdBy: 'antinuke-system',
                    includeRoles:    true,
                    includeChannels: true,
                    includeSettings: true,
                    includeBans:     false,
                }).catch(e => this.bot.logger.warn(`[AntiNuke] Post-restore backup failed: ${e.message}`));
            }
