...
class SecurityDashboard {
    constructor(bot) {
        ...
        // Google OAuth configuration (set GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET in env)
        this.googleConfig = {
            clientId: process.env.GOOGLE_CLIENT_ID || null,
            clientSecret: process.env.GOOGLE_CLIENT_SECRET || null,
            redirectUri: process.env.GOOGLE_REDIRECT_URI || `${domain}/auth/google/callback`,
        };
...
The GuardianBot Team`
    ...
    setupRoutes() {
        ...
        this.app.get('/dashboard', this.authenticateToken.bind(this), (req, res) => {
            console.log('\n========== /dashboard ROUTE HIT ==========');
        ...
        // Public stats endpoint (no auth required) — used by /site/ landing page
        this.app.get('/api/public/stats', async (req, res) => {
            try {
                let threatsBlocked = 0;
                let totalMembers = 0;
                let guildsProtected = 0;
                const responseTimeMs = this.bot?.client?.ws?.ping ?? Math.round(40 + Math.random() * 20);

                if (this.bot?.database) {
                    try {
                        const threatRow = await this.bot.database.get(
                            `SELECT COUNT(*) as c FROM action_logs WHERE action_type IN ('BAN','KICK','TIMEOUT','WARN','DELETE_MESSAGE','MUTE')`
                        );
                        threatsBlocked = threatRow?.c || 0;
                    } catch { /* ignore */ }

                    try {
                        const memberRow = await this.bot.database.get(
                ...
                if (this.bot?.client?.guilds?.cache) {
                    guildsProtected = this.bot.client.guilds.cache.size;
                    // Sum actual member counts if DB gave 0
                    if (!totalMembers) {
                        totalMembers = this.bot.client.guilds.cache.reduce((sum, g) => sum + (g.memberCount || 0), 0);
                    }
                }
                ...
                res.json({ success: false, threatsBlocked: 0, totalMembers: 0, guildsProtected: 0, responseTimeMs: 0 });
            }
        });
        ...
        this.app.get('/auth/google/callback', this.handleGoogleCallback.bind(this));
...
/* Theme applies to bot dashboard and site only, not admin panel */`;
        ...
        // Server Backup API routes (list, create, delete, verify — used by /setup/antinuke)
        try {
            const createBackupRoutes = require('./routes/backups');
            this.app.use('/api', createBackupRoutes(this));
        } catch (err) {
            this.bot?.logger?.warn && this.bot.logger.warn('[Dashboard] Backup routes failed to load:', err?.message || err);
        }
    ...
    async authenticateToken(req, res, next) {
        ...
        if (!token) {
            // Redirect browser HTML requests to login; return JSON for API calls
    ...
    async handleDiscordCallback(req, res) {
        try {
            ...
            console.log('[OAuth Callback] Cookie options:', {
                httpOnly: true,
                secure: cookieSecure,
                sameSite: 'lax', // lax required for OAuth cross-site redirect chains
                maxAge: 24 * 60 * 60 * 1000,
                path: '/'
            });
            ...
            res.cookie('dashboardToken', token, {
                httpOnly: true,
                secure: cookieSecure,
                sameSite: 'lax', // lax required: Discord redirects back cross-site, strict blocks the cookie
                maxAge: 24 * 60 * 60 * 1000,
                path: '/'
            });
    ...
    async handleGoogleAuth(req, res) {
        if (!this.googleConfig || !this.googleConfig.clientId) {
            this.bot?.logger?.warn && this.bot.logger.warn('Google OAuth attempted but GOOGLE_CLIENT_ID is not configured');
            return res.redirect('/login?error=' + encodeURIComponent('Google login is not configured on this instance'));
        }

        const nonce = require('crypto').randomBytes(16).toString('hex');
        const stateSecret = process.env.OAUTH_STATE_SECRET || process.env.JWT_SECRET;
        if (!stateSecret) {
            return res.redirect('/login?error=' + encodeURIComponent('OAuth is not properly configured'));
        }
        const state = jwt.sign(
            { n: nonce, ts: Date.now(), ip: req.ip, ua: req.headers['user-agent'] || '', provider: 'google' },
            stateSecret,
            { expiresIn: '10m' }
        );
        ...
        authUrl.searchParams.set('redirect_uri', this.googleConfig.redirectUri);
    ...
    async handleGoogleCallback(req, res) {
        try {
            ...
            if (oauthError) {
                return res.redirect('/login?error=' + encodeURIComponent(oauthError === 'access_denied' ? 'Google sign-in was cancelled' : `Google OAuth error: ${oauthError}`));
            }
            ...
            try {
                const stateSecret = process.env.OAUTH_STATE_SECRET || process.env.JWT_SECRET;
                if (!stateSecret) return res.status(500).send('OAuth is not properly configured');
                decodedState = jwt.verify(state, stateSecret);
            } catch (e) {
                return res.status(403).send('Invalid OAuth state - verification failed');
            }

            if (!decodedState?.ts || Date.now() - Number(decodedState.ts) > 600000) {
                return res.status(403).send('OAuth state expired - please try again');
            }
            if (decodedState.provider !== 'google') {
                return res.status(403).send('Invalid OAuth state provider');
            }
            ...
            this.bot.logger.info('Google OAuth: Exchanging code for token...');
            ...
            const gUser = userResponse.data;
            // gUser: { sub, email, name, given_name, family_name, picture, email_verified }
            this.bot.logger.info(`Google OAuth: User ${gUser.email} authenticated`);

            if (!gUser.email_verified) {
                return res.redirect('/login?error=' + encodeURIComponent('Google account email is not verified'));
            }

            const entitlement = await this.getUserPlan(`google:${gUser.sub}`).catch(() => ({ plan: 'free', isPremium: false }));

            // Google users get 'user' role; they can upgrade via access-code page (same as non-admin Discord users)
            const token = jwt.sign(
                {
                    userId: `google:${gUser.sub}`,
                    username: gUser.name || gUser.email,
                    globalName: gUser.name || gUser.email,
                    avatar: null, // no Discord avatar
                    email: gUser.email,
            ...
            this.bot.logger.info(`Google OAuth: JWT issued for ${gUser.email}, redirecting to /access-code`);
            res.redirect('/access-code');
        } catch (error) {
            this.bot.logger.error('Google OAuth error:', error.response?.data || error.message);
            let errorMessage = 'Google authentication failed';
            if (error.response?.status === 400) errorMessage = 'Invalid Google authorization code or redirect URI';
            else if (error.response?.status === 401) errorMessage = 'Invalid Google client credentials';
            res.redirect(`/login?error=${encodeURIComponent(errorMessage)}`);
        }
    }
  ...
  <div class="card">
    ...
    async getFullAnalytics(req, res) {
        try {
            ...
            for (let i = days - 1; i >= 0; i--) {
                ...
                _dateRange.push(d.toISOString().split('T')[0]);
            }

            // If we still don't have a guildId, or database is unavailable,
            // just return the empty analytics payload instead of an error.
            if (!guildId || !this.bot.database) {
                // Still fill date range so frontend charts render axes
                analytics.memberActivity = _dateRange.map(date => ({ date, joins: 0, leaves: 0, messages: 0 }));
                analytics.commandUsage = _dateRange.map(date => ({ date, count: 0 }));
                analytics.moderationTrend = _dateRange.map(date => ({ date, warns: 0, kicks: 0, bans: 0, timeouts: 0 }));
            ...
            try {
                ...
                const dateSet = new Set([..._dateRange, ...joinData.map(d => d.date), ...leaveData.map(d => d.date)]);
                ...
                analytics.memberActivity = sortedDates.map(date => ({
                    ...
                    messages: 0  // filled in later from message_analytics
                }));
            ...
            try {
                ...
                // Fill full date range so charts always show all days
                analytics.commandUsage = _dateRange.map(date => ({
                    date,
                    count: commandData.find(d => d.date === date)?.count || 0
                }));
            ...
            try {
                // 4. Message Analytics (hourly distribution + daily counts)
                ...
                // Daily message counts for charts
                const dailyMsgData = await this.bot.database.all(`
                    SELECT DATE(created_at) as date, SUM(message_count) as messages
                    FROM message_analytics 
                    WHERE guild_id = ? AND created_at > datetime('now', '-${days} days')
                    GROUP BY DATE(created_at)
                    ORDER BY date ASC
                ...
                // Merge daily message counts into memberActivity
                for (const row of dailyMsgData) {
                    const entry = analytics.memberActivity.find(m => m.date === row.date);
                    if (entry) {
            ...
            try {
                ...
                // 5b. Moderation Trend — daily breakdown by type
                const modTrend = await this.bot.database.all(`
                ...
                analytics.moderationTrend = _dateRange.map(date => {
                    const dayRows = modTrend.filter(r => r.date === date);
                    const entry = { date, warns: 0, kicks: 0, bans: 0, timeouts: 0 };
                    for (const row of dayRows) {
                        const t = (row.action_type || '').toLowerCase();
                        if (t.includes('warn')) entry.warns += row.count;
                        else if (t.includes('kick')) entry.kicks += row.count;
                        else if (t.includes('ban')) entry.bans += row.count;
                        else if (t.includes('timeout') || t.includes('mute')) entry.timeouts += row.count;
                    }
                    return entry;
                });
    ...
    async getUserServers(req, res) {
        try {
            ...
            // Google-authenticated users have no Discord identity — return empty server list
            if (req.user?.provider === 'google') {
                this.bot.logger.info(`[SERVERS] Google user ${userId} — no Discord guilds available`);
    ...
    async start(port = process.env.PORT || process.env.DASHBOARD_PORT || 3001) {
        ...
        if (!this._routesSetup && typeof this.setupRoutes === 'function') {
            try { this.setupRoutes(); this._routesSetup = true; } catch (e) { /* ignore */ }
        }
