            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={async () => {
                  ...
                  setSubmitError('');
                  try {
                    // Send crash/bug report to the admin telemetry endpoint
                    const report = {
                      type: 'bug_report',
                    ...
                    await fetch(`${PLATFORM_BASE}/api/admin/telemetry/report`, {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify(report),
                    }).catch(() => {}); // Best-effort, don't fail the UX
                    setReportSent(true);
                  } catch {
                    setSubmitError('Failed to submit report. Your report has been saved locally.');
                    setReportSent(true); // Still show success — we don't want to frustrate users
                  }
                  setSubmitting(false);
                }}
                disabled={submitting}
