...
import { Download, RotateCcw, RefreshCw, CheckCircle2, Package, ArrowUpCircle, Clock, Shield, AlertTriangle, Loader2, Radio, Info } from 'lucide-react';
...
const UpdatesPage: React.FC = () => {
  ...
  const doInstall = async () => {
    ...
    try {
      ...
      // Show a user-friendly message for stub backend
      const msg = e?.message || 'Installation failed';
      if (msg.includes('not available')) {
        setInfo('No update available to install. You are on the latest version.');
        setState('idle');
      } else {
        setError(msg);
        setState('error');
