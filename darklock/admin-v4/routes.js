...
router.post('/announcements', MW.modOrAbove, MW.auditLog('announcements'), async (req, res) => {
  try {
    ...
    try {
      ...
      await _db.run(`
        INSERT OR IGNORE INTO updates (id, title, version, type, content, published_at, created_by, created_at)
        VALUES (?, ?, ?, 'minor', ?, ?, ?, ?)
...
router.get('/security', MW.adminOrAbove, async (req, res) => {
  try {
    ...
    res.json({
      ...
      activeSessions:      sessions?.count || 0,
      lockdownLevel:       config.lockdown_level?.value || null,
    });
  } catch (err) {
...
const DARKLOCK_ROOT = '/mnt/nvme/discord-bot';

function runCmd(cmd) {
  return new Promise((resolve) => {
    exec(cmd, { cwd: DARKLOCK_ROOT }, (err, stdout, stderr) => {
...
function spawnDetached(cmd, args, logFile) {
  try {
    const out = fs.openSync(`${DARKLOCK_ROOT}/${logFile}`, 'a');
    const p = spawn(cmd, args, { detached: true, stdio: ['ignore', out, out], cwd: DARKLOCK_ROOT, env: process.env });
    p.unref();
  } catch (e) { console.error('[Lockdown] spawn failed:', cmd, e.message); }
}

// POST /security/lockdown  — activate soft / medium / hard lockdown
router.post('/security/lockdown', MW.ownerOrCoowner, MW.auditLog('security'), async (req, res) => {
  try {
    const { level } = req.body;
    if (!['soft', 'medium', 'hard'].includes(level)) {
      return res.status(400).json({ success: false, error: 'Invalid lockdown level. Use soft, medium, or hard.' });
    }

    const now = new Date().toISOString();
    const scope = 'darklock_site';
    const messages = {
      soft:   'Scheduled maintenance in progress. We\'ll be back shortly.',
      medium: 'Platform undergoing security lockdown. Services temporarily offline.',
      hard:   'Emergency lockdown active. All services are offline.',
    };

    // 1. Enable maintenance mode (all levels)
    const existing = await db.get(`SELECT id FROM maintenance_state WHERE scope = ?`, [scope]);
    if (!existing) {
      const id = crypto.randomUUID();
      await db.run(`INSERT INTO maintenance_state (id, scope, enabled, admin_bypass, title, message) VALUES (?, ?, 1, 1, 'Lockdown', ?)`, [id, scope, messages[level]]);
    } else {
      await db.run(`UPDATE maintenance_state SET enabled = 1, message = ?, updated_by = ?, updated_at = ? WHERE scope = ?`, [messages[level], req.admin.id, now, scope]);
    }

    // 2. Force logout all users + disable registration (medium + hard)
    if (level === 'medium' || level === 'hard') {
      await Q.forceLogoutAllUsers();
      await Q.setConfig('registration_enabled', 'false', req.admin.email);
    }

    // 3. Stop Discord bot (medium + hard)
    if (level === 'medium' || level === 'hard') {
      await runCmd('pkill -f "node src/bot.js" || true');
    }

    // 4. Stop hardware bridges + guard (hard only)
    if (level === 'hard') {
      await runCmd('pkill -f "pico_guild_display/pico_bridge\|pico_led_bridge\|elegoo_bridge\|hardware_controller" || true');
      await runCmd('pkill -f "guard-v2\|guard.*rust\|guard.*watchdog" || true');
    }

    // Save current lockdown level so release knows what to restart
    await Q.setConfig('lockdown_level', level, req.admin.email);

    await Q.logAudit({ admin_id: req.admin.id, admin_email: req.admin.email, action: `lockdown_${level}`, category: 'security', ip_address: MW.getClientIP(req) });
    res.json({ success: true, message: `${level.charAt(0).toUpperCase() + level.slice(1)} lockdown activated.`, level });
  } catch (err) {
    console.error('[Admin v4] Lockdown error:', err);
    res.status(500).json({ success: false, error: 'Lockdown failed: ' + err.message });
  }
});

// POST /security/lockdown/release  — lift lockdown and restore services
router.post('/security/lockdown/release', MW.ownerOrCoowner, MW.auditLog('security'), async (req, res) => {
  try {
    const config = await Q.getConfig();
    const level = config.lockdown_level?.value || 'soft';
    const now = new Date().toISOString();
    const scope = 'darklock_site';

    // 1. Disable maintenance mode
    await db.run(`UPDATE maintenance_state SET enabled = 0, updated_by = ?, updated_at = ? WHERE scope = ?`, [req.admin.id, now, scope]);

    // 2. Re-enable registration
    await Q.setConfig('registration_enabled', 'true', req.admin.email);

    // 3. Restart Discord bot (medium + hard)
    if (level === 'medium' || level === 'hard') {
      spawnDetached('node', ['src/bot.js'], 'logs/bot.log');
    }

    // 4. Restart hardware bridges (hard only)
    if (level === 'hard') {
      setTimeout(() => {
        spawnDetached('python3', ['hardware/pico_guild_display/pico_bridge.py'], 'logs/pico_guild.log');
        spawnDetached('python3', ['hardware/pico_led_bridge.py'], 'logs/pico_led.log');
        spawnDetached('python3', ['hardware/elegoo_guild_display/elegoo_bridge.py'], 'logs/elegoo_bridge.log');
        spawnDetached('python3', ['hardware_controller.py'], 'logs/hardware.log');
      }, 2000);
    }

    // 5. Clear lockdown state
    await Q.setConfig('lockdown_level', '', req.admin.email);

    await Q.logAudit({ admin_id: req.admin.id, admin_email: req.admin.email, action: 'lockdown_release', category: 'security', ip_address: MW.getClientIP(req) });
    res.json({ success: true, message: 'Lockdown released. Services are coming back online.' });
  } catch (err) {
    console.error('[Admin v4] Release lockdown error:', err);
    res.status(500).json({ success: false, error: 'Failed to release lockdown: ' + err.message });
  }
});
