...
class DarklockDatabase {
    ...
    async runMigrations() {
        ...
        try {
            await this.run(`ALTER TABLE users ADD COLUMN banner TEXT`);
            console.log('[Darklock DB] Migration: Added banner column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN two_factor_backup_codes TEXT`);
            console.log('[Darklock DB] Migration: Added two_factor_backup_codes column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN two_factor_enabled_at TEXT`);
            console.log('[Darklock DB] Migration: Added two_factor_enabled_at column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN timezone TEXT DEFAULT 'UTC'`);
            console.log('[Darklock DB] Migration: Added timezone column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN preferences TEXT`);
            console.log('[Darklock DB] Migration: Added preferences column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN notifications TEXT`);
            console.log('[Darklock DB] Migration: Added notifications column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN oauth_provider TEXT`);
            console.log('[Darklock DB] Migration: Added oauth_provider column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`ALTER TABLE users ADD COLUMN oauth_id TEXT`);
            console.log('[Darklock DB] Migration: Added oauth_id column');
        } catch (err) {
            // Column already exists
        }

        try {
            await this.run(`CREATE INDEX IF NOT EXISTS idx_users_oauth ON users(oauth_provider, oauth_id)`);
        } catch (err) {
    ...
    async updateUser(userId, updates) {
        ...
        const allowedFields = [
            ...
            'avatar', 'banner', 'two_factor_enabled', 'two_factor_secret',
            'two_factor_backup_codes', 'two_factor_enabled_at',
            'last_login', 'last_login_ip', 'password_changed_at',
            'settings', 'preferences', 'notifications',
            'active', 'language', 'timezone',
