...
class SevenSegmentDisplay:
    ...
    def run(self):
        """
        Main loop: non-blocking character-by-character serial read + continuous
        digit multiplexing.  Using readline() here would block if data arrives
        in chunks and freeze all 4 digits off — never do that.
        """
        buf = ""
        digit = 0
        while True:
            # Non-blocking: check if data waiting FIRST, then read ONE char
                    ...
                    if ch == '\n':
                        ...
                        if line.startswith("COUNT:"):
                            try:
                                ...
                                print(f"[Pico] Count: {self.current_number}")
                            except ValueError:
                                pass
                        elif line == "PING":
                            print("[Pico] PONG")
            ...
            # Multiplex: show ONE digit per iteration so serial never blocks display
            self.display_digit(digit, self._digits()[digit])
            digit = (digit + 1) % 4
...
if __name__ == "__main__":
    ...
    # Startup test: show 8888 for ~2 seconds using the multiplexed loop
    print("Testing 8888...")
    display.current_number = 8888
    start = utime.ticks_ms()
    digit = 0
    while utime.ticks_diff(utime.ticks_ms(), start) < 2000:
        display.display_digit(digit, display._digits()[digit])
        digit = (digit + 1) % 4
        utime.sleep_us(2000)
